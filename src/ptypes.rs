...
pub enum Item {
    ...
    Import(ImportDecl),
    Using(UsingDecl),
    AttributeConfig(AttributeConfigDecl),
    MacroDecl(MacroDecl),
...
#[derive(Debug, Clone)]
// note: standalone attributed config block item
pub struct AttributeConfigDecl {
    // note: raw @attrs in source order
    pub attributes: Vec<String>,
    // note: config payload expression (record literal)
    pub config: Expr,
}
